...
public class KabinengewichtDocument : PdfBaseDocument
{
    ...
    void TableCarweights(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Row(2).Column(1).Background(secondaryVariantColor).Text("Kabinengewicht gesamt");
            table.Cell().Row(2).Column(2).PaddingRight(80, Unit.Millimetre).Background(secondaryVariantColor).AlignRight().Text($"{Math.Round(CarWeightResult.KabinenGewichtGesamt)} kg  ");
